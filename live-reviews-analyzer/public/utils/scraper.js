...
function extractReviewData(container) {
  ...
  if (reviewDateElement) {
    console.log(reviewDateElement.textContent.trim());
...
async function scrollAndExtractReviews() {
  ...
  while (true) {
    ...
    if (scrollableElement.scrollHeight === lastScrollHeight) {
      ...
      console.log("Pushing", reviewContainers.length);
    ...
    console.log("Scrolling", document.querySelectorAll(SELECTORS.REVIEW_CONTAINER).length);
...
function dateParser(date) {
  ...
  return mappings[unit] ? mappings[unit] : {};
}
